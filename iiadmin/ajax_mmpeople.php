...
class MMInstructorsAjax extends AdminMultimediaPage
{
	...
	{	parent::MMLoggedinConstruct();
		switch ($_GET['action'])
		{	case 'popup':
				echo $this->ListPossibleInstructors();
				break;
			case 'refresh':
				echo $this->multimedia->PeopleListTable();
				break;
			case 'add':
				if ($this->multimedia->AddInstructor($_GET['inid']))
				{	echo '<div class="successmessage">New person added</div>';
				}
				echo $this->ListPossibleInstructors();
				break;
			case 'remove':
				$this->multimedia->RemoveInstructor($_GET['inid']);
				echo $this->multimedia->PeopleListTable();
				break;
		}
	...
	{	ob_start();
		...
		{	echo '<table>';
			...
			{	$instructor = new Instructor($instructor_row);
				...
				echo '</td><td>', $this->InputSafeString($instructor->GetFullName()), '</td><td><a onclick="InstructorAdd(', $this->multimedia->id, ',', $instructor->id, ')">add</a></td></tr>';
			}
			echo '</table>';
		} else
	...
	{	$instructors = array();
		$existing = $this->multimedia->GetPeople();
...
} // end of defn MMInstructorsAjax

$ajax = new MMInstructorsAjax();
?>