...
{	public $items = array();
					...
					{	case 'course': // if item is course, add bookings
							...
							{	foreach ($attendees as $attendee)
								{	if (!$userid = (int)$attendee['userid'])
									{	$student = new Student();
										$userid = $student->CreateFromAttendee($attendee);
									}
									if ($userid)
									{	if ($bookid = $this->CreateBookingFromItem($item, $p, new Student($userid)))
	...
	public function CreateBookingFromItem($item = array(), $product = false, $student = false)
	{	$booking = new CourseBooking();
		if($booking->CreateFromOrderItem($student, $product, $item))
		{	$booking->SendStudentEmail();
			return $booking->id;
		}
