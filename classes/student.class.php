...
{	public $details = array();
	...
	public function CreateFromAttendee($data = array())
	{	
		...
		{	$fields = array();
			...
			{	if ($this->db->AffectedRows())
				{	$this->Get($this->db->InsertID());
					$this->SendCreatedRegEmail($pword);
				}
			//	if (SITE_TEST) $fail[] = $sql;
	...
	public function SendCreatedRegEmail($pword = '')
	{	if ($this->ValidEmail($this->details['username']))
		{	
			...
			$mailtemplate = new MailTemplate('reg_created');
			$mail = new HTMLMail();
