...
{	
	...
	{	
		...
		{	// display chosen multimedia
			...
			echo '<div class="mmDisplay"><h1><span', $this->multimedia->details['socialbar'] ? ' class="headertextWithSM"' : '', '>', $this->InputSafeString($this->multimedia->details['mmname']), '</span>', $this->multimedia->details['socialbar'] ? $this->GetSocialLinks(3, true) : '', '</h1><div class="mmdOutput">', $this->multimedia->Output(695), '</div><div class="mmdDesc">';
			if ($people_text = $this->multimedia->GetAuthorText())
			{	echo '<p>', $people_text, '</p>';
			}
			echo $this->multimedia->DisplayFullDescHTML(400), '</div></div>';
		} else
