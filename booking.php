...
{	
	...
	{	echo '<div id="bookingDetails"><div><span>Booking ref.</span><div class="bdContent">';
		if ($order = $this->booking->GetOrder())
		{	echo date('@H:i \o\n j F Y', strtotime($order['orderdate']));
			if (($gifter = $this->booking->IsGift()) && $gifter->id)
			{	echo ', Gifted by: ', $this->InputSafeString($gifter->GetName());
			}
		}
		echo '</div><div class="clear"></div></div><div><span>Course Details</span><div class="bdContent"><a href="', $this->booking->course->Link(), '">view course</a></div><div class="clear"></div></div><div><span>Amount</span><div class="bdContent">&pound;', number_format($this->booking->order_item['totalpricetax'], 2), '</div><div class="clear"></div></div><div><span>Course/Event date/time</span><div class="bdContent">', $this->booking->course->DateDisplayForDetails('<br />', 'D. jS F, Y', $date_sep = ' - ', $time_sep = '<br />'), '</div><div class="clear"></div></div><div><span>Venue</span><div class="bdContent">', $this->booking->course->GetVenue()->details['vname'], '<br />', $this->booking->course->GetVenue()->GetAddress();
